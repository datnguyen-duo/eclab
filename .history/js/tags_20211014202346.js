    ...
    $.fn.inputTags = function (options) {
          ...
          if (element) {
            switch (typeof element) {
              ...
              case "object":
                ...
                if (
                  "[object Object]" === Object.prototype.toString.call(element)
                ) {
      ...
      if ("object" === typeof options || !options) {
        ...
        var obj = this.each(function () {
          ...
          /* Constantes */
          self.UNIQID = Math.round(
            Date.now() / (Math.random() * (548 - 54) - 54)
          );
          ...
          self.ITEM_CONTENT =
            '<span class="value" title="Cliquez pour éditer">%s</span><i class="close-item">&times</i>';
          ...
          self.AUTOCOMPLETE_LIST_CLASS =
            self.DEFAULT_CLASS + "-autocomplete-list";
              ...
              if (self.$input.hasClass("is-edit")) {
                ...
                if (self._autocomplete()._isSet()) {
                  if (self._autocomplete()._get("only")) {
                    if (
                      $.inArray(value, self._autocomplete()._get("values")) < 0
                    ) {
          ...
          self._concatenate = function () {
            if (
              !"boolean" === typeof self.options.max ||
              self.options.max > 0
            ) {
              if (self.options.tags.length > self.options.max) {
                self.options.tags.splice(
                  -Math.abs(self.options.tags.length - self.options.max)
                );
              }
            }
              ...
              _build: function () {
                ...
                self
                  ...
                  .forEach(function (v, k) {
                    var $item =
                      $.inArray(v, self.tags) > 0
                        ? $(
                            self.AUTOCOMPLETE_ITEM_CONTENT.replace("%s", v)
                ...
                self.$autocomplete
                  .off("click")
                  .on("click", "." + self.AUTOCOMPLETE_ITEM_CLASS, function () {
                    var $item = $(this);
          ...
          self._errors = function (type) {
            ...
            self._displayErrors(
              self.options.errors[type].replace("%s", self.options[type]),
              type
            );
          ...
          self._displayErrors = function (error, type) {
            var $error = $(self.ERROR_CONTENT.replace("%s", error)).attr(
              "data-error",
              type
            );
            ...
            if (
              $("." + self.ERROR_CLASS + '[data-error="' + type + '"]').length
            ) {
              return false;
            }
          ...
          self._isSet = function (elem) {
            return "undefined" === typeof self.options[elem] ||
              false === self.options[elem] ||
              self.options[elem].length <= 0
              ? false
              : true;
          };
          ...
          self._callMethod = function (method_name, self) {
            if (
              "undefined" === typeof self.options[method_name] ||
              "function" !== typeof self.options[method_name]
            ) {
              return false;
            }

            self.options[method_name].apply(
              this,
        ...
        $.error(
          "[undefined method] Erreur: La méthode [" +
            options +
            "] que vous tentez d'appeler n'existe pas."
        );
      }
    };
