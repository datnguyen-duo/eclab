(function($) {
    ...
    if ($('.page-template-template-home').length) {
        ...
        $.fn.inputTags.defaults = {
            ...
            editable: true,
            autocomplete: {
                values: ['#RighToCareIL', '#Families', '#ILoveMotherNature'],
                only: false
            },
            errors: {
        ...
        var tags = $('#tags').inputTags({        
            ...
            init: function(elem) {
                ...
                $('<p class="results">').html('<strong>Your Tags:</strong> ' + elem.tags.join(' - ')).insertAfter(elem.$list);
            },
            create: function() {
            ...
            change: function(elem) {
                $('.results').empty().html('<strong>Your Tags:</strong> ' + elem.tags.join(' - '));
            }
        });
    }
